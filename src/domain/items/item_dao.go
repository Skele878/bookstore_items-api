...
func (i *Item) Get() rest_errors.RestErr {
	...
	if err != nil {
		...
		return rest_errors.NewInternalServerError(fmt.Sprintf("error when trying to get id %s", i.Id), errors.New("darabase error"))
	}

	bytes, err := result.Source.MarshalJSON()
	if err != nil {
		return rest_errors.NewInternalServerError("error when trying to parse datavase response", errors.New("database error"))
	}
...
func (i *Item) Search(query queries.EsQuery) ([]Item, rest_errors.RestErr) {
	...
	if len(items) == 0 {
		return nil, rest_errors.NewNotFoundError("no items found matchong givem criteria")
	}
